...
namespace genvrp {
    ...
    public:
        ...
        /** Path to the shared SQLite database, if any (used to cooperate with other algorithms). */
        std::string pathToSharedDb;

        /** Interval between two DB updates for the current best solution (seconds). */
        double dbUpdateIntervalSec = 0.0;

        /** Global start time of the cooperative run (seconds since epoch, same基准 as Python time.time()).
         *  Only used to align the runningtime written into the shared DB with other algorithms.
         *  If zero or negative, HGS-TV will fall back to its own internal elapsed time.
         */
        double globalStartTimeSec = 0.0;
    ...
    public:
        ...
        ParamsFactory& withCommandLine(const CommandLineParser& c) {
            ...
            withSharedDbFile(c.getSharedDbPath());
            withDbUpdateIntervalSec(c.getUpdateIntervalSec());
            withGlobalStartTimeSec(c.getStartTimeSec());
        ...
        /** Sets the shared SQLite DB path in the params. */
        ParamsFactory& withSharedDbFile(std::string dbFile) {
            p.pathToSharedDb = std::move(dbFile);
            return *this;
        }

        /** Sets the global start time (seconds since epoch) in the params. */
        ParamsFactory& withGlobalStartTimeSec(double t0) {
            p.globalStartTimeSec = t0;
            return *this;
        }

        /** Sets the DB update interval (seconds). */
        ParamsFactory& withDbUpdateIntervalSec(int sec) {
            p.dbUpdateIntervalSec = static_cast<double>(sec);
            return *this;
        }
