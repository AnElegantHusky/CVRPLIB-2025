...
namespace genvrp {
    ...
    void CommandLineParser::displayHelp() const {
        ...
        std::cout << "\t-sharedDB dbPath\n"
                  << "\t\tPath to a shared SQLite database for cooperation between algorithms.\n"
                  << "\t\tIf provided, HGS-TV will try to read an initial solution from it and\n"
                  << "\t\twill append/update its best solutions during the search.\n\n";
        std::cout << "\t-startTime t0\n"
                  << "\t\tGlobal start time of the cooperative run, in seconds since epoch (same as Python time.time()).\n"
                  << "\t\tUsed only to align the runningtime written to the shared DB with other algorithms.\n\n";
        std::cout << "\t-updateInterval sec\n"
                  << "\t\tInterval in seconds between two DB updates for the current best solution.\n"
                  << "\t\tIf not provided or non-positive, DB updates are disabled.\n\n";
        std::cout << "\t-t timeout\n"
        ...
        if(argc % 2 != 0 || argc < 2) {
            ...
            for(int i = 2; i < argc; i += 2) {
                if(std::string(argv[i]) == "-t") {
                    ...
                    bssPath = argv[i + 1];
                } else if(std::string(argv[i]) == "-sharedDB") {
                    sharedDbPath = argv[i + 1];
                } else if(std::string(argv[i]) == "-updateInterval") {
                    updateIntervalSec = std::atoi(argv[i + 1]);
                } else if(std::string(argv[i]) == "-startTime") {
                    startTimeSec = std::atof(argv[i + 1]);
                } else {
