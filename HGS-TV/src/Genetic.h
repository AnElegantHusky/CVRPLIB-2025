...
#include "solution/HgsDbAdapter.h"
...
namespace genvrp {
    ...
    class Genetic {
        ...
        /** Helper to inject an initial individual built from a set of routes. */
        void injectInitialRoutes(const std::vector<std::vector<int>>& routes);
    ...
    public:
        ...
        Genetic(Params& params) :
            ...
            // If a shared DB is configured, also try to inject the current global best
            // solution from the DB as an initial elite individual.
            if(!this->params.pathToSharedDb.empty()) {
                std::vector<std::vector<int>> dbRoutes;
                double dbScore = 0.0;

                HgsDbConfig cfg{this->params.pathToSharedDb, "HGS-TV"};
                if(loadBestRoutesFromDb(cfg, dbRoutes, dbScore)) {
                    injectInitialRoutes(dbRoutes);
                }
            }
        }
    };
