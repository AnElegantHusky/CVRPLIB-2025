...
def make_config(one_day_sec):
    ...
    return {
        ...
        # "initial_solution": EXPERIMENT_PATH / "Initial_sols" / "XL-n1234-k55" / "20260112_235936.sol",  # path to a .sol file; None means no warm-start seed
        ...
        "tier2": [
            {"name": "WS_AILS2_5d", "jar": ROOT_PATH / "AILS2" / "target" / "AILS-II-1.0-SNAPSHOT-jar-with-dependencies.jar",
        ...
        "tier3": [
            {"name": "E_AILS2_10d", "type": "ails", "jar": ROOT_PATH / "AILS2" / "target" / "AILS-II-1.0-SNAPSHOT-jar-with-dependencies.jar",
...
def main():
    ...
    for tier in ("tier1", "tier2"):
        for cfg in CONFIG[tier]:
            cmd_map[cfg["name"]] = build_ails_cmd(
                cfg["jar"], instance_path, shared_db_path,
                start_time, shared_params, writing_interval, cfg["extra_args"]
            )

    for cfg in CONFIG["tier3"]:
        if cfg["type"] == "ails":
            cmd_map[cfg["name"]] = build_ails_cmd(
                cfg["jar"], instance_path, shared_db_path,
                start_time, shared_params, writing_interval, cfg["extra_args"]
            ...
            cmd_map[cfg["name"]] = build_filo_cmd(
                cfg["exe"], instance_path, shared_db_path,
                start_time, max_running_sec, writing_interval
            )
        elif cfg["type"] == "hgs":
            cmd_map[cfg["name"]] = build_hgs_cmd(
                cfg["exe"], instance_path, shared_db_path,
                start_time, max_running_sec, writing_interval
            )
