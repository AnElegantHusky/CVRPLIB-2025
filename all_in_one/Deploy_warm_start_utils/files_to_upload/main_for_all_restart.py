...
EXTERNAL_SOL_DIR_NAME = "ails2_restart_sols"
...
def inject_initial_solution(instance_name, db_path):
    """
    从 ails2_restart_sols 读取解并注入 DB
    """
    # 构造寻找路径: ROOT/ails2_restart_sols/InstanceName/**/*.sol
    # 使用 glob 递归查找，因为中间层可能是 {fixed_method_name}
    search_pattern = ROOT_PATH / "SISR" / EXTERNAL_SOL_DIR_NAME / instance_name / "**" / "*.sol"
...
if __name__ == '__main__':
    ...
    # max_running_time_min = 20  # : ONLY FOR DEBUG
    ...
    ails_configs = [
        ...
        {'name': f'AILS2_15day', 'etaMax': 1, 'limit': one_day_time_sec * 15},
    ...
    ails_eoh_acc_cmd = [                            # : AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
        "-jar", java_cp_eoh_acc,
        "-file", instance_path.as_posix(),
    ...
    ails_eoh_acc_large_cmd = [  # : AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
    ...
    ails_eoh_omega_cmd = [                            # : AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
    ...
    manage_workers(process_dict,
                   ...
                   ails_eoh_ruin_template=ails_eoh_ruin_cmd)
