...
def load_best(db_path):
    try:
        # timeout 设置为 10秒，读取操作通常很快
        with sqlite3.connect(db_path, timeout=10.0) as conn:
            # 开启 Row 工厂模式，这样可以通过列名访问数据 (row['score'])，而不只是索引 (row[0])
            conn.row_factory = sqlite3.Row

            # 不需要事务，普通的 SELECT 即可
            cursor = conn.execute(
                "SELECT score, solution, algo_name, runningtime FROM global_best WHERE id=1"
            )
            row = cursor.fetchone()

            if row:
                raw_sol = row['solution']
                score = row['score']

                if raw_sol and score < 1e19:
                    try:
                        return {
                            "score": score,
                            "solution": json.loads(raw_sol),  # 关键：将 JSON 字符串转回 Python 对象(List/Dict)
                            "algo_name": row['algo_name'],
                            "runningtime": row['runningtime']
                        }
                    except json.JSONDecodeError as e:
                        print(f"[Data Error] JSON decode failed in load_best: {e}")
                        return None

            # 如果是初始状态 (score=1e20) 或没有行
            return None

    except sqlite3.OperationalError as e:
        print(f"[DB Read Locked] Failed to load best: {e}")
        return None
    except Exception as e:
...
def sisr_cvrp(
    ...
    def ruin(last_routes: list[list[int]], neighbours: list[list[int]], in_absents=None, isHugeRuin=False) -> tuple[
        list[int], list[int]]:
        """
    ...
    def recreate_single_pickup_demand_hard_time_window(data, distance_matrix, current_routes, absents,
                                                       last_length=None):
        """Algorithm 3 from SISR paper
        Managing route feasibility is done in this step for: Capacity, Time Window
        data have columns:
    ...
    def recreate_single_pickup_demand_soft_time_window(data, distance_matrix, current_routes, absents,
                                                       last_length=None):
        """Algorithm 3 from SISR paper
        Managing route feasibility is done in this step for: Capacity, Time Window
        data have columns:
        ...
        def check_constraints(complete_r, _print=False):
            ...
            for j in range(0, len(complete_r) - 1):
                ...
                if demand_current < 0:
                    if _print:
                        print("     DITOLAK. Route ini tidak memiliki barang untuk memenuhi demand di node {}".format(
                            complete_r[j + 1]))
                    return False
                if pickup_current > 200:
                    if _print:
                        print(
                            "     DITOLAK. Route ini tidak dapat menampung pickup di node {}".format(complete_r[j + 1]))
                    return False
    ...
    def fleet_min(n, data, distance_matrix, neighbours, routes, verbose_step):
        ...
        for i in range(n):
            ...
            current_routes, new_absents = recreate(data, distance_matrix, current_routes, new_absents,
    ...
    init_route_pre = load_best(shared_db)
    init_route = init_route_pre['solution'] if init_route_pre is not None else None
    ...
    #         print()
    #
    #         if (i_iter+1) % (verbose_step*10) == 0 and plot_figures:
    ...
    for route, color in zip(routes, colors):
        ...
        for i in range(len(route) - 1):
            plt.plot(
                ...
                label="Route: {}".format(route) if i == 0 else None
                # to prevent multiple legend for the same route/color.
            )
