...
def manage_workers(process_dict, sisr_args, ails_common_args, ails_configs, filo_template, hgs_template, ails_eoh_acc_template,  ails_eoh_omega_template, ails_eoh_omega_acc_template,ails_eoh_omega2_template,best_info, start_time):  # add new template
    """
    智能管理进程重启。
    ...
    if not process_dict.get('ails2_eoh_omega2') or not process_dict['ails2_eoh_omega2'].is_alive():       #add omega
        # 构建该特定配置的命令
        p = multiprocessing.Process(
            target=run_external_process,
