...
def manage_workers(process_dict, sisr_args, ails_common_args, ails_configs, filo_template, hgs_template, ails_eoh_acc_template,  ails_eoh_omega_template, ails_eoh_omega_acc_template,best_info, start_time):  # add new template
    """
    智能管理进程重启。
...
if __name__ == '__main__':
    ...
    # limit_min = 5
    reading_interval_sec = args.update_interval_sec
    ...
    ails_eoh_omega_acc_cmd = [                            # TODO: AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
        "-jar", java_cp_eoh_omega,
        "-file", instance_path.as_posix(),
    ...
    filo_cmd = [
        ...
        "--update-interval-seconds", str(writing_interval_sec)
    ]
    ...
    manage_workers(process_dict, sisr_args, ails_cmd, ails_configs, filo_cmd, hgs_cmd, ails_eoh_acc_cmd, ails_eoh_omega_cmd, ails_eoh_omega_acc_cmd,{}, start_time)
        ...
        while True:
            ...
            try:
                ...
                if global_best_info and crt_best_info['score'] > global_best_info['score']:
                    ...
                    manage_workers(process_dict, sisr_args, ails_cmd, ails_configs, filo_cmd, hgs_cmd, ails_eoh_acc_cmd, ails_eoh_omega_cmd, ails_eoh_omega_acc_cmd,global_best_info, start_time)
            except Exception as e:
                # 4. 详细异常信息输出
                ...
                error_detail = (
                    ...
                    f"当前crt_best_dist值: {crt_best_info['score']}\n"  # 上下文变量
            ...
            time.sleep(reading_interval_sec)  # 轮询间隔，不要太频繁
