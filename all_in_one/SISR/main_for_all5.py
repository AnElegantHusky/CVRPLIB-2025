...
java_cp_eoh_ruin_list = [
    ROOT_PATH / 'AILS2_Eoh_Ruin' / 'target' / 'AILS-II-1.0-SNAPSHOT-jar-with-dependencies.jar',      # TODO
...
AILS2_EoH_Ruin_PATH = ROOT_PATH / 'AILS2_Eoh_Ruin'        # TODO 1 增加新方法目录
...
def manage_workers(process_dict,
    ...
    if ails_eoh_ruin_template:
        if not process_dict.get('ails2_eoh_ruin') or not process_dict['ails2_eoh_ruin'].is_alive():       # TODO:
            # 构建该特定配置的命令
            p = multiprocessing.Process(
                target=run_external_process,
...
if __name__ == '__main__':
    ...
    ails_eoh_ruin_cmd = [  # TODO: AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
    ...
    ails_eoh_omega2_cmd = [                            # TODO: AILSII不同版本新增cmd
        "java",
        # "--enable-native-access=ALL-UNNAMED", # handle the warning
        "-jar", java_cp_eoh_omega,
        "-file", instance_path.as_posix(),
