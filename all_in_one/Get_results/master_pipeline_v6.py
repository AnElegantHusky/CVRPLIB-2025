...
def run_cycle():
        ...
        try:
            with Connection(host=srv['host'], user=srv['user'], port=srv['port'],
                ...
                if res.ok and res.stdout.strip():
                    db_files = res.stdout.strip().split('\n')
                    for remote_f in db_files:
            ...
            msg = f"⚠️ Error: {e}"
            server_status_report.append(f"🖥️ {srv['host']}: {msg}")
            error_servers += 1
    ...
    with open(rep_p, 'w', encoding='utf-8') as f:
        ...
        if down_servers > 0: f.write(f"* 🛑 离线警告: 有 {down_servers} 台服务器程序停止运行！\n")
        if error_servers > 0: f.write(f"* ⚠️ 连接异常: 有 {error_servers} 台服务器无法连接。\n")
        f.write("-" * 60 + "\n\n")
        ...
        f.write(f"📊 【Part 4】未突破 BKS 的差距排行 (按Gap%排序):\n" + "-" * 60 + "\n" + "\n".join(
