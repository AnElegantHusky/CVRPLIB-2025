...
EXPORT_CSV = False  # True: 提取并压缩CSV; False: 跳过CSV生成（提升速度）
...
FETCH_INTERVAL = 1 * 3600  # 巡检间隔：1小时
...
def clean_old_data(dir_path):
    if dir_path.exists():
        try:
            ...
            log_with_time(f"⚠️ 清理目录 {dir_path.name} 失败 (可能被占用): {e}")
...
def get_cvrplib_instances_float():
    ...
    log_with_time("🌍 正在获取 CVRPLIB 最新世界纪录 (BKS)...")
    try:
        # verify=False 解决部分环境下的 SSL 证书问题
...
def run_cycle():
    ...
    log_with_time(f"🚀 周期开始: {ts_human} (导出CSV: {'开启' if EXPORT_CSV else '关闭'})")
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                if row and row['solution']:
                    ...
                    # 没破 BKS，但突破了自我，进入 Part 2
                    elif is_improved_locally:
                        local_history[instance_name] = score
                        ...
                        if bks_val:
                            msg += f" | Gap to BKS: +{score - bks_val:.4f}"
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("\n".join(bks_breakers) if bks_breakers else "暂无破纪录结果。\n")

        f.write(f"\n\n🚀【Part 2】仅优于本地历史的提升 ({len(local_only_improvements)} 个):\n" + "-" * 60 + "\n")
        f.write("\n".join(local_only_improvements) if local_only_improvements else "暂无本地提升。\n")

    log_with_time(f"✨ 周期结束。报告: {report_file.name}")


def main():
    log_with_time(f"🚀 系统启动。正在进入周期为 {FETCH_INTERVAL // 3600}小时 的巡检模式...")
    while True:
        try:
            run_cycle()
