...
//	---------CSV OUTPUT----------
	...
	{ 
        ...
        this.logPath = reader.getLogPath()+"/best.csv";
        this.sharedCSV = reader.getSharedCSV();
        ...
        this.initSolutionList=reader.getInitSolutionList();
	...
	public void evaluateSolution()
	{
		if((solution.f-bestF)<-epsilon) {
            ...
            if (print) {
                ...
                // .csv output
                String[] ails2record = {""+timeAF, ""+bestF, ""+bestSolution.toListString()};
                String[] sharedSecord = {"ails2", ""+timeAF, ""+bestF, ""+bestSolution.toListString()};

                try (FileWriter fw = new FileWriter(this.logPath, true);
                     CSVPrinter printer = new CSVPrinter(fw, CSVFormat.DEFAULT)) {
                    printer.printRecord(ails2record);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }

                try (FileWriter fw = new FileWriter(this.sharedCSV, true);
                     CSVPrinter printer = new CSVPrinter(fw, CSVFormat.DEFAULT)) {
                    printer.printRecord(sharedSecord);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
            }
