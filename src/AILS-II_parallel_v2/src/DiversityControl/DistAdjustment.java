...
public class DistAdjustment {
    ...
    public void distAdjustment(long totalWorkerCpuTime) {
        if (iterator == 0) {
            ini = threadMXBean.getCurrentThreadCpuTime(); // 获取当前线程的CPU时间
            this.totalWorkerCpuTime = 0;
        }
        ...
        switch (stoppingCriterionType) {
            case Iteration:
                ...
                break;
            case Time:
                this.totalWorkerCpuTime = totalWorkerCpuTime;
                timeAdjustment();
                break;
            default:
                break;
    ...
    private void timeAdjustment() {
        double current = (double) (threadMXBean.getCurrentThreadCpuTime() - ini + totalWorkerCpuTime) / 1_000_000_000; // 转换为秒
