...
#             adding_position = sorted(probable_place, key=lambda x: x[-1])[0]
    ...
    # create log file
    inst_log_path = inst_log_path
    os.makedirs(inst_log_path, exist_ok=True)

    best_csv = os.path.join(inst_log_path, 'best.csv')
    all_csv = os.path.join(inst_log_path, 'all.csv')

    csv_path = '{}.csv'.format('{}/{}'.format(inst_log_path, inst_name))
...
#     if n_iter_fleet is None: n_iter_fleet=int(max(n_iter*0.1, 1))
    ...
    for i_iter in range(n_iter):
        ...
        append_csv_row(all_csv, [i_iter, crt_running_time, current_distance, current_routes])
        ...
        if len(current_routes) < len(best_routes) or \
            ...
            if len(current_routes) < len(best_routes) or current_distance < best_distance:
                ...
                append_csv_row(best_csv, [i_iter, crt_running_time, best_distance, best_routes])
