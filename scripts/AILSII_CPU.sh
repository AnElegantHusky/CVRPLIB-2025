...
for i in {0..7}; do
    ...
    for j in {1..1}; do     
        ...
        java -jar -Xms2000m -Xmx4000m bin/AILSII_CPU.jar \
             -file XLDemo/${inst}.vrp \
             -rounded true \
             -stoppingCriterion Time \
             -limit ${time[$i]} \
             -best ${best[$i]} \
        > results/AILSII_CPU/${inst}.csv

        # 添加日志信息
        echo "Completed run $j for instance $inst, results saved to results/AILSII_a/${inst}.csv"
    done
done